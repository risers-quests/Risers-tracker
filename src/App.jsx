...
export default function RisersTracker() {
  ...
  const st = {
    ...
    tab: (active) => ({ padding: "8px 20px", background: active ? T.parchment : "transparent", color: active ? T.forest : T.mist, border: "none", borderRadius: "8px 8px 0 0", fontFamily: "Inter, sans-serif", fontWeight: active ? 700 : 500, fontSize: 13, cursor: "pointer" }),
    ...
    btn: (variant = "primary") => ({ padding: "9px 20px", borderRadius: 8, border: "none", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 13, cursor: "pointer", background: variant === "primary" ? T.forest : variant === "danger" ? T.danger : T.mist, color: variant === "ghost" ? T.ink : "#fff" }),
      ...
      <div>
        <div style={st.card}>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            ...
            {editingEntry && (
              <button style={st.btn("ghost")} onClick={() => { setEditingEntry(null); setEntryForm(emptyEntry()); }}>Cancel</button>
            )}
        ...
        {kidRecords.length === 0 ? (
          ...
          Object.entries(questGroups).map(([questName, entries]) => (
            <div key={questName} style={{ marginBottom:24 }}>
              <div style={{ fontFamily:"'Fraunces',serif", fontSize:16, fontWeight:700, color:T.amber, marginBottom:10 }}>
                🗺 {questName}
              </div>
              {entries.map((entry) => {
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:10, marginBottom:16 }}>
        {tempKids.map((k, i) => (
          <input key={i} style={st.input} value={k}
            onChange={e => { const next=[...tempKids]; next[i]=e.target.value; setTempKids(next); }} />
        ))}
      </div>
      <div style={{ display:"flex", gap:10, alignItems:"center" }}>
        <button style={st.btn("primary")} onClick={() => { setKids([...tempKids]); flashSaved(); }}>Save Names</button>
        {saved && <span style={{ fontSize:12, color:T.moss, fontWeight:700 }}>✓ Saved</span>}
      </div>
